...
def main():
    ...
    try:
        ...
        if trust_store.get(peer_name) is None:
            ...
            print(f"[alice] '{peer_name}' is not yet trusted.")
            print(f"[alice] their identity fingerprint is: {fp}")
            print(
                f"[alice] compare this, out-of-band (in person, on a call, "
            ...
            answer = (
                input(f"[alice] trust and pin this identity as '{peer_name}'? [y/N]: ")
                .strip()
                .lower()
            )
            if answer != "y":
                print(f"[alice] declined to trust '{peer_name}'. Aborting.")
                sock.close()
                return
            trust_store.pin(peer_name, peer_pubkey)
            trust_store.save(f"{KEY_DIR}/alice_trust.json")
            print(f"[alice] TOFU: pinned new identity '{peer_name}' ({fp})")
